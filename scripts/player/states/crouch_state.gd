...
func handle_input() -> void:
	...
	if player.is_charge_just_pressed() and player.combat.can_start_charge():
		...
		return
	if player.is_attack_just_pressed() and player.combat.can_start_melee(player.combat.crouch_stamina_cost):
		state_machine.transition_to("attack_crouch")
